	...
	while (cal < cal_num){
		switch (Path) {
			...
			{	
				...
				ODM_Write4Byte(pDM_Odm, 0xcb8, 0x00100000);// cb8[20] Â±N SI/PI Â¨?Â¥??vÂ¤?ÂµÂ¹ iqk_dpk module
				cal_retry = 0;
				while(1){
					// one shot
...
*/
			...
			if (VDF_enable == 1){
						...
						case 0:
                {
                    // one shot
						...
						ODM_Write4Byte(pDM_Odm, 0xeb8, 0x00100000);// cb8[20] Â±N SI/PI Â¨?Â¥??vÂ¤?ÂµÂ¹ iqk_dpk module
						cal_retry = 0;
						while(1){
							// one shot
							ODM_Write4Byte(pDM_Odm, 0x980, 0xfa000000);
			...
			if (TX1IQKOK == FALSE)
				...
				ODM_SetBBReg(pDM_Odm, 0xee8, BIT(31), 0x1);    // TX VDF Enable
					...
					ODM_Write4Byte(pDM_Odm, 0xeb8, 0x00100000);// cb8[20] ???SI/PI ä½¿ç¨æ¬?????iqk_dpk module
